...
import com.xiaody.springboot.util.Utils;
...
public class MySqlSessionFactoryBean
	...
	private static final String BASE_MAPPER_TEMPLATE = "base_mapper.tpl";

	private static final String BASE_MODEL_PLACEHOLDER = "#MODEL_CLASS_NAME#";

	private static final String TABLE_NAME_PLACEHOLDER = "#TABLE_NAME#";

	private static final Pattern MAPPER_CLASS_NAME = Pattern.compile("<mapper namespace=\"(.*?Mapper)\">");
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		if (!isEmpty(this.mapperLocations)) {
			for (Resource mapperLocation : this.mapperLocations) {
				...
				try {
					InputStream inputStream = buildMapperConfiguration(mapperLocation);
	...
	private InputStream buildMapperConfiguration(Resource mapperLocation) throws Exception {
		String tableName = mapperLocation.getFilename().replace(".xml", "");
		logger.debug("Entity table name is [" + tableName + "].");
        String baseMapperTemplate = Utils.readFileContent(BASE_MAPPER_TEMPLATE);

        File injectedMapperConfig = File.createTempFile(mapperLocation.getFilename(), "");
        ...
        while ((line = reader.readLine()) != null) {
            content.append(line).append(Utils.getLineSeparator());

            if (tableName.startsWith("std_")) {
                logger.debug("Skip hijacking standard mybatis mapper configuration [" + mapperLocation.getFilename() + "]");
                continue;
            }

            if (Utils.trim(line).startsWith("<resultMap id=\"defaultResultMap\""))
                throw new RuntimeException("[defaultResultMap] is reserved result map.");

            if (Utils.trim(line).startsWith("<insert id=\"create\""))
                throw new RuntimeException("[create] is reserved insert method.");

            if (Utils.trim(line).startsWith("<select id=\"get\""))
                throw new RuntimeException("[get] is reserved select method.");

            if (Utils.trim(line).startsWith("<update id=\"update\""))
                throw new RuntimeException("[update] is reserved update method.");

            if (Utils.trim(line).startsWith("<delete id=\"delete\""))
                throw new RuntimeException("[delete] is reserved delete method.");

            if (Utils.trim(line).startsWith("<select id=\"exists\""))
                throw new RuntimeException("[exists] is reserved select method.");

            if (!Utils.trim(line).startsWith("<mapper namespace="))
                continue;

            // inject base mapper methods
            String mapperClassName = Utils.find(MAPPER_CLASS_NAME, line);
            Class<?> mapperClass = Class.forName(mapperClassName);

            if (mapperClass == null)
                throw new RuntimeException("Mapper class can't be automatically detected.");

            logger.debug("Mapper class is [" + mapperClassName + "].");

            // assume all mapper classes extends from BaseMapper
            Type baseMapperType = null;
            for (Type type : mapperClass.getGenericInterfaces()) {
                if (type.toString().contains(BaseMapper.class.getName())) {
                    baseMapperType = type;
                    break;
                }
            }
            ...
            if (baseMapperType == null)
                throw new RuntimeException("Mapper doesn't extends from [BaseMapper].");

            // BaseMapper has only one generic type for model class
            Class<?> modelClass = (Class<?>) (((ParameterizedType) baseMapperType).getActualTypeArguments()[0]);

            if (modelClass == null)
                throw new RuntimeException("Model class can't be automatically detected.");

            logger.debug("Model class is [" + modelClass.getName() + "].");

            String baseModelMethods = baseMapperTemplate.replaceAll(BASE_MODEL_PLACEHOLDER, modelClass.getName());
            baseModelMethods = baseModelMethods.replaceAll(TABLE_NAME_PLACEHOLDER, tableName);

            content.append(baseModelMethods).append(Utils.getLineSeparator());
        }

        Utils.appendFile(injectedMapperConfig, content.toString());

        return new FileInputStream(injectedMapperConfig);
	}
